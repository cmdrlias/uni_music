...
class ShowArtistAlbumsState extends State<ShowArtistAlbums> {
  ...
  void initState() {
    ...
    print("Seus álbuns são " + albumsNames.toString());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) => ListTile(
          title: albumsNames[index] != null ? Text(albumsNames[index]) : Text("Álbum desconhecido"),
          onTap: () {
            Navigator.of(context).push(MaterialPageRoute(
...
class ShowAlbumState extends State<ShowAlbum> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: widget.albumName != null ? Text(widget.albumName) : Text("Álbum desconhecido"),
      ),
      body: ListView.builder(
